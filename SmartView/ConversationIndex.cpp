...
void ConversationIndex::Parse()
{
	...
	m_Parser.GetBYTE(&b1);
	m_Parser.GetBYTE(&b2);
	m_guid.Data2 = (unsigned short)((b1 << 8) | b2);
	m_Parser.GetBYTE(&b1);
	m_Parser.GetBYTE(&b2);
	m_guid.Data3 = (unsigned short)((b1 << 8) | b2);
	m_Parser.GetBYTESNoAlloc(sizeof(m_guid.Data4), sizeof(m_guid.Data4), m_guid.Data4);

	if (m_Parser.RemainingBytes() > 0)
	{
		m_ulResponseLevels = (ULONG)m_Parser.RemainingBytes() / 5; // Response levels consume 5 bytes each
	}
	...
	if (m_lpResponseLevels)
	{
		...
		ULONG i = 0;
		for (i = 0; i < m_ulResponseLevels; i++)
		{
			m_Parser.GetBYTE(&b1);
			m_Parser.GetBYTE(&b2);
			m_Parser.GetBYTE(&b3);
			...
			m_Parser.GetBYTE(&b1);
			m_lpResponseLevels[i].Random = (BYTE)(b1 >> 4);
			m_lpResponseLevels[i].Level = (BYTE)(b1 & 0xf);
		}
	}
}
...
_Check_return_ wstring ConversationIndex::ToStringInternal()
{
	...
	wstring szGUID = GUIDToString(&m_guid);
