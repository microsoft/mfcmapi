...
namespace SmartViewTest
{
	...
	private:
		...
		static const bool parseAll = false;
		...
		void AreEqualEx(const wstring& expected, const wstring& actual, const wchar_t* message = nullptr, const __LineInfo* pLineInfo = nullptr)
		{
			if (expected != actual)
			{
				if (message != nullptr)
		...
		vector<SmartViewTestData> loadTestData(std::initializer_list<SmartViewTestResource> resources)
		{
