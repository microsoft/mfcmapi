...
$inDir = "$rootDir\SmartViewTestData\In"
$outDir = "$rootDir\SmartViewTestData\Out"
$outputHeaderFile = "$rootDir\res\SmartViewTests.h"
...
foreach ($file in $inDirfiles) {
    ...
    if ($name -match $inDirPattern) { 
        ...
        $defines += $define
        $resource = "$name TEXTFILE ""$($file.Name)"""
        $resources += $resource
...
foreach ($file in $outDirFiles) {
    ...
    if ($name -match $outDirPattern) {
        ...
        $define = "#define $name $index"
        $defines += $define
        $resource = "$name TEXTFILE ""$($file.FullName)"""
        $resources += $resource
    }
}
...
#define TEST(PARSERTYPE, NAME, NUM) \
	TEST_METHOD(PARSERTYPE##_##NUM) { unittest::test(TEST##NAME##PARSERTYPE, IDR_SV##NAME##NUM##IN, IDR_SV##NAME##NUM##OUT); }
