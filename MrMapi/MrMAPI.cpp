...
void DisplayUsage(BOOL bFull)
{
	...
	if (bFull)
	{
		...
		printf(
			"           You can also pass a string, which will load the first MAPI whose path contains the string.\n");
		printf("\n");
...
#define CHECKFLAG(__flag) ((pRunOpts->ulMAPIMIMEFlags & (__flag)) == (__flag))
		...
		if (CHECKFLAG(MAPIMIME_TOMIME) && (CHECKFLAG(MAPIMIME_CHARSET) || CHECKFLAG(MAPIMIME_UNICODE))) return false;
		break;
	case cmdmodeProfile:
