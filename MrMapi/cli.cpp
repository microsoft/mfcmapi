...
namespace cli
{
	...
	void DisplayUsage(BOOL bFull)
	{
		...
		printf(
			"   MrMAPI [-%s] [-%s] [-%s] [-%s <type>] <property number>|<property name>\n",
		...
		printf(
			"   MrMAPI -%s <type> -%s <input file> [-%s] [-%s <output file>]\n",
		...
		printf(
			"   MrMAPI -%s <flag value> [-%s] [-%s] <property number>|<property name>\n",
			g_Switches[switchFlag].szSwitch,
			g_Switches[switchDispid].szSwitch,
			g_Switches[switchDecimal].szSwitch);
		printf("   MrMAPI -%s <flag name>\n", g_Switches[switchFlag].szSwitch);
		printf(
			"   MrMAPI -%s [-%s <profile>] [-%s <folder>]\n",
		...
		printf(
			...
			g_Switches[switchFolder].szSwitch);
		printf(
			"   MrMAPI -%s | -%s [-%s <profile>] [-%s <folder>] [-%s <output directory>]\n",
			...
			g_Switches[switchOutput].szSwitch);
		printf(
			"          [-%s <subject>] [-%s <message class>] [-%s] [-%s] [-%s <count>] [-%s]\n",
		...
		printf(
			...
			g_Switches[switchFolder].szSwitch);
		printf(
			"   MrMAPI -%s -%s <path to input file> -%s <path to output file> [-%s]\n",
			g_Switches[switchXML].szSwitch,
			g_Switches[switchInput].szSwitch,
			g_Switches[switchOutput].szSwitch,
			g_Switches[switchSkip].szSwitch);
		printf(
			"   MrMAPI -%s [fid] [-%s [mid]] [-%s <profile>]\n",
			g_Switches[switchFid].szSwitch,
			g_Switches[switchMid].szSwitch,
			g_Switches[switchProfile].szSwitch);
		printf(
			"   MrMAPI [<property number>|<property name>] -%s [<store num>] [-%s <profile>]\n",
			g_Switches[switchStore].szSwitch,
			g_Switches[switchProfile].szSwitch);
		printf(
			"   MrMAPI [<property number>|<property name>] -%s <folder> [-%s <profile>]\n",
			g_Switches[switchFolder].szSwitch,
			g_Switches[switchProfile].szSwitch);
		printf(
			"   MrMAPI -%s -%s <folder> [-%s <profile>]\n",
			g_Switches[switchSize].szSwitch,
			g_Switches[switchFolder].szSwitch,
			g_Switches[switchProfile].szSwitch);
		printf(
			"   MrMAPI -%s | -%s -%s <path to input file> -%s <path to output file> [-%s <conversion flags>]\n",
		...
		if (bFull)
		{
			...
			printf(
				"   -N   (or -%s) Number is in decimal. Ignored for non-numbers.\n",
				g_Switches[switchDecimal].szSwitch);
			...
			printf(
				"   -E   (or -%s) Map an error code to its name and vice versa.\n", g_Switches[switchError].szSwitch);
			printf("           May be combined with -S and -N switches.\n");
			printf("\n");
			printf("   Smart View Parsing:\n");
			printf(
				"   -P   (or -%s) Parser type (number). See list below for supported parsers.\n",
				g_Switches[switchParser].szSwitch);
			printf(
				"   -B   (or -%s) Input file is binary. Default is hex encoded text.\n",
				g_Switches[switchBinary].szSwitch);
			printf("\n");
			printf("   Rules Table:\n");
			...
			printf(
				"   -C   (or -%s) Output contents table. May be combined with -H. Profile optional.\n",
				g_Switches[switchContents].szSwitch);
			printf(
				"   -H   (or -%s) Output associated contents table. May be combined with -C. Profile optional\n",
				g_Switches[switchAssociatedContents].szSwitch);
			...
			printf(
				"   -L   (or -%s) List details to screen and do not output files.\n", g_Switches[switchList].szSwitch);
			printf(
				"   -Re  (or -%s) Restrict output to the 'count' most recent messages.\n",
				g_Switches[switchRecent].szSwitch);
			printf("\n");
			printf("   Child Folders:\n");
			printf(
				"   -Chi (or -%s) Display child folders of selected folder.\n",
				g_Switches[switchChildFolders].szSwitch);
			printf("\n");
			...
			printf(
				"           If -%s is specified without a FID, search/display all folders\n",
				g_Switches[switchFid].szSwitch);
			printf("   -Mid (or -%s) Message ID (MID) to search for.\n", g_Switches[switchMid].szSwitch);
			printf(
				"           If -%s is specified without a MID, display all messages in folders specified by the FID "
				"parameter.\n",
				g_Switches[switchMid].szSwitch);
			printf("\n");
			...
			printf(
				"   -Ma  (or -%s) Convert an EML file to MAPI format (MSG file).\n", g_Switches[switchMAPI].szSwitch);
			printf(
				"   -Mi  (or -%s) Convert an MSG file to MIME format (EML file).\n", g_Switches[switchMIME].szSwitch);
			printf(
				"   -I   (or -%s) Indicates the input file for conversion, either a MIME-formatted EML file or an MSG "
				"file.\n",
				g_Switches[switchInput].szSwitch);
			printf(
				"   -O   (or -%s) Indicates the output file for the conversion.\n", g_Switches[switchOutput].szSwitch);
			printf(
				"   -Cc  (or -%s) Indicates specific flags to pass to the converter.\n",
				g_Switches[switchCCSFFlags].szSwitch);
			...
			printf(
				"   -Rf  (or -%s) (MAPI->MIME only) Indicates the EML should be generated in RFC822 format.\n",
				g_Switches[switchRFC822].szSwitch);
			printf("           If not present, RFC1521 is used instead.\n");
			printf(
				"   -W   (or -%s) (MAPI->MIME only) Indicates the maximum number of characters in each line in the\n",
				g_Switches[switchWrap].szSwitch);
			printf("           generated EML. Default value is 74. A value of 0 indicates no wrapping.\n");
			printf(
				"   -En  (or -%s) (MAPI->MIME only) Indicates the encoding type to use. Supported values are:\n",
				g_Switches[switchEncoding].szSwitch);
			printf("              1 - Base64\n");
			printf("              2 - UUENCODE\n");
			...
			printf("              5 - 8bit\n");
			printf(
				"   -Ad  (or -%s) Pass MAPI Address Book into converter. Profile optional.\n",
				g_Switches[switchAddressBook].szSwitch);
			printf(
				"   -U   (or -%s) (MIME->MAPI only) The resulting MSG file should be unicode.\n",
				g_Switches[switchUnicode].szSwitch);
			printf(
				"   -Ch  (or -%s) (MIME->MAPI only) Character set - three required parameters:\n",
				g_Switches[switchCharset].szSwitch);
			...
			printf(
				"   -B   (or -%s) If specified, profile section guid is byte swapped.\n",
				g_Switches[switchByteSwapped].szSwitch);
			printf(
				"   -O   (or -%s) Indicates the output file for profile export.\n", g_Switches[switchOutput].szSwitch);
			printf("           Required if a profile is specified.\n");
			printf("\n");
			printf("   Receive Folder Table\n");
			printf(
				"   -%s Displays Receive Folder Table for the specified store\n",
				g_Switches[switchReceiveFolder].szSwitch);
			printf("\n");
			...
			printf(
				"   -F   (or -%s) Folder to scan. Default is Inbox. See list below for supported folders.\n",
				g_Switches[switchFolder].szSwitch);
			...
			printf(
				"   -M   (or -%s) More properties. Tries harder to get stream properties. May take longer.\n",
		...
		for (auto i = 1; i < argc; i++)
		{
			...
			if (!bHitError) switch (iSwitch)
				...
				case switchType:
					...
					{
						pRunOpts->ulTypeNum =
							interpretprop::PropTypeNameToPropType(strings::LPCSTRToWstring(argv[i + 1]));
						i++;
					}
					break;
				case switchFlag:
...
#define CHECKFLAG(__flag) ((pRunOpts->MAPIMIMEFlags & (__flag)) == (__flag))
			...
			if (CHECKFLAG(MAPIMIME_TOMIME) && (CHECKFLAG(MAPIMIME_CHARSET) || CHECKFLAG(MAPIMIME_UNICODE)))
				return false;
			break;
		case cmdmodeProfile:
