	...
	void EntryIdStruct::parseBlocks()
	{
		...
		else if (EIDStructType::messageDatabase == m_ObjectType)
		{
			...
			case MDB_STORE_EID_V2_MAGIC:
			{
				...
				addChild(labeledBlock(L"Reserved Bytes = ", m_MessageDatabaseObject.v2Reserved));
			}
			break;
			case MDB_STORE_EID_V3_MAGIC:
			{
		...
		else if (EIDStructType::folder == m_ObjectType)
		{
			...
			addChild(labeledBlock(L"GlobalCounter = ", m_FolderOrMessage.FolderObject.GlobalCounter));
			terminateBlock();

			addChild(labeledBlock(L"Pad = ", m_FolderOrMessage.FolderObject.Pad));
		}
		else if (EIDStructType::message == m_ObjectType)
		{
			...
			addChild(labeledBlock(L"Folder GlobalCounter = ", m_FolderOrMessage.MessageObject.FolderGlobalCounter));

			addChild(labeledBlock(L"Pad1 = ", m_FolderOrMessage.MessageObject.Pad1));
			...
			addChild(labeledBlock(L"Message GlobalCounter = ", m_FolderOrMessage.MessageObject.MessageGlobalCounter));

			addChild(labeledBlock(L"Pad2 = ", m_FolderOrMessage.MessageObject.Pad2));
		}
	}
} // namespace smartview