...
namespace mapi::processor
{
	...
	void mapiProcessor::ProcessMailboxTable(_In_ const std::wstring& szExchangeServerName)
	{
		...
		if (lpPrimaryMDB && store::StoreSupportsManageStore(lpPrimaryMDB)) do
			{
				auto lpMailBoxTable = store::GetMailboxTable(lpPrimaryMDB, szExchangeServerName, ulOffset);
				if (lpMailBoxTable)
				{
	...
	void mapiProcessor::ProcessMessage(_In_ LPMESSAGE lpMessage, bool bHasAttach, _In_opt_ LPVOID lpParentMessageData)
	{
		if (!lpMessage) return;
	...
	void mapiProcessor::DoMailboxTablePerRowWork(_In_ LPMDB /*lpMDB*/, _In_ const _SRow* /*lpSRow*/, ULONG /*ulCurRow*/)
	{
	}
