...
namespace output
{
	...
	void outputFormInfo(dbgLevel ulDbgLvl, _In_opt_ FILE* fFile, _In_ LPMAPIFORMINFO lpMAPIFormInfo)
	{
		...
		if (lpPropVals)
		{
			outputProperties(ulDbgLvl, fFile, ulPropVals, lpPropVals, lpMAPIFormInfo, false);
		...
		{
			outputFormPropArray(ulDbgLvl, fFile, lpMAPIFormPropArray);
	...
	void outputTable(dbgLevel ulDbgLvl, _In_opt_ FILE* fFile, _In_ LPMAPITABLE lpMAPITable)
	{
		...
		for (;;)
		{
			...
			for (ULONG iCurRow = 0; iCurRow < lpRows->cRows; iCurRow++)
			{
				...
				outputSRow(ulDbgLvl, fFile, &lpRows->aRow[iCurRow], nullptr);
		...
		_In_opt_ LPMAPIPROP lpObj)
	{
		...
		for (ULONG i = 0; i < cNotify; i++)
		{
			...
			case fnevCriticalError:
				...
				outputBinary(ulDbgLvl, fFile, sbin);
			...
			case fnevTableModified:
				...
				outputProperty(ulDbgLvl, fFile, &lpNotifications[i].info.tab.propIndex, lpObj, false);
				Outputf(ulDbgLvl, fFile, true, L"lpNotifications[%u].info.tab.propPrior = \n", i);
				outputProperty(ulDbgLvl, fFile, &lpNotifications[i].info.tab.propPrior, nullptr, false);
				Outputf(ulDbgLvl, fFile, true, L"lpNotifications[%u].info.tab.row = \n", i);
				outputSRow(ulDbgLvl, fFile, &lpNotifications[i].info.tab.row, lpObj);
				break;
			...
			case fnevSearchComplete:
				...
				outputPropTagArray(ulDbgLvl, fFile, lpNotifications[i].info.obj.lpPropTagArray);
				break;
			case fnevIndexing:
				Outputf(
					ulDbgLvl,
		...
		bool bRetryStreamProps)
	{
		...
		if (lpSortedProps)
		{
			...
			{
				outputProperty(ulDbgLvl, fFile, &lpSortedProps[i], lpObj, bRetryStreamProps);
			}
		}
	...
	void outputSRowSet(dbgLevel ulDbgLvl, _In_opt_ FILE* fFile, _In_ LPSRowSet lpRowSet, _In_opt_ LPMAPIPROP lpObj)
	{
		...
		if (lpRowSet->cRows >= 1)
		{
			...
			{
				outputSRow(ulDbgLvl, fFile, &lpRowSet->aRow[i], lpObj);
			}
		}
	}
		...
		_In_opt_ LPMAPIPROP lpObj)
	{
		...
		if (!lpRes)
		{
			Output(ulDbgLvl, fFile, true, L"outputRestriction called with NULL lpRes!\n");
			return;
		}
