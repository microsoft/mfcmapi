...
namespace mapi::mapimime
{
		...
		_In_opt_ LPADRBOOK lpAdrBook)
	{
		...
		if (SUCCEEDED(hRes) && lpConverter)
		{
			...
			hRes =
				EC_H(file::MyOpenStreamOnFile(MAPIAllocateBuffer, MAPIFreeBuffer, STGM_READ, lpszEMLFile, &lpEMLStm));
				...
				if (SUCCEEDED(hRes) && lpMimeStm)
				{
					...
					if (SUCCEEDED(hRes))
					{
						...
						hRes = EC_H(file::MyOpenStreamOnFile(
							MAPIAllocateBuffer, MAPIFreeBuffer, STGM_CREATE | STGM_READWRITE, lpszEMLFile, &lpFileStm));
		...
		bool bUnicode)
	{
		...
		{
			hRes = EC_H(
				ImportEMLToIMessage(lpszEMLFile, pMessage, convertFlags, bApply, hCharSet, cSetApplyType, lpAdrBook));
			if (SUCCEEDED(hRes))
			{
