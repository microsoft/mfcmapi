...
//Copies an SRestriction into new memory, allocating space for sub structures as needed
HRESULT CopyRestriction(LPSRestriction* lpDestRes, LPSRestriction lpSrcRes, LPVOID lpParent)
{
	if (!lpDestRes || !lpSrcRes) return MAPI_E_INVALID_PARAMETER;
	HRESULT hRes = S_OK;
	LPVOID lpAllocParent = lpParent;
	if (lpParent)
	{
	...
	else
	{
		EC_H(MAPIAllocateBuffer(
			sizeof(SRestriction),
			(LPVOID *) lpDestRes));
		lpAllocParent = *lpDestRes;
	}
	if (S_OK == hRes)
	{
		EC_H(CopyRestriction(*lpDestRes,lpSrcRes,lpAllocParent));
	}
	return hRes;
}

//Copies an SRestriction into existing, allocated memory, allocating space for sub structures as needed
HRESULT CopyRestriction(LPSRestriction lpDestRes, LPSRestriction lpSrcRes, LPVOID lpParent)
{
	if (!lpDestRes || !lpSrcRes || !lpParent) return MAPI_E_INVALID_PARAMETER;
	HRESULT hRes = S_OK;
	memcpy(lpDestRes,lpSrcRes,sizeof(SRestriction));
	switch(lpDestRes->rt)
	{
	...
	case RES_AND:
		{
			if (lpSrcRes->res.resAnd.lpRes && lpDestRes->res.resAnd.cRes < ULONG_MAX/sizeof(SRestriction))
			{
				EC_H(MAPIAllocateMore(
					sizeof(SRestriction) * lpDestRes->res.resAnd.cRes,
					lpParent,
					(LPVOID*)&lpDestRes->res.resAnd.lpRes));
				if (lpDestRes->res.resAnd.lpRes)
				{
					ULONG i = 0;
					for (i = 0 ; i < lpDestRes->res.resAnd.cRes ; i++)
					{
						EC_H(CopyRestriction(
							&lpDestRes->res.resAnd.lpRes[i],
							&lpSrcRes->res.resAnd.lpRes[i],
							lpParent));
					}
				}
			}
	...
	case RES_NOT:
		{
			if (lpSrcRes->res.resNot.lpRes)
				EC_H(CopyRestriction(&lpDestRes->res.resNot.lpRes,lpSrcRes->res.resNot.lpRes,lpParent));
		}
		break;
	...
	case RES_CONTENT:
		{
			if (lpSrcRes->res.resContent.lpProp)
			{
				EC_H(MAPIAllocateMore(
					sizeof(SPropValue),
					lpParent,
					(LPVOID*)&lpDestRes->res.resContent.lpProp));
				if (lpDestRes->res.resContent.lpProp)
				{
					EC_H(PropCopyMore(
						lpDestRes->res.resContent.lpProp,
						lpSrcRes->res.resContent.lpProp,
						MAPIAllocateMore,
						lpParent));
				}
			}
	...
	case RES_SUBRESTRICTION:
		{
			if (lpSrcRes->res.resSub.lpRes)
				EC_H(CopyRestriction(&lpDestRes->res.resSub.lpRes,lpSrcRes->res.resSub.lpRes,lpParent));
		}
		break;
	case RES_COMMENT:
		{
			if (lpSrcRes->res.resComment.lpRes)
				EC_H(CopyRestriction(&lpDestRes->res.resComment.lpRes,lpSrcRes->res.resComment.lpRes,lpParent));

			if (lpSrcRes->res.resComment.lpProp && lpDestRes->res.resComment.cValues < ULONG_MAX/sizeof(SPropValue))
			{
				EC_H(MAPIAllocateMore(
					sizeof(SPropValue) * lpDestRes->res.resComment.cValues,
					lpParent,
					(LPVOID*)&lpDestRes->res.resComment.lpProp));
				if (lpDestRes->res.resComment.lpProp)
				{
					ULONG i = 0;
					for (i = 0 ; i < lpDestRes->res.resComment.cValues ; i++)
					{
						EC_H(PropCopyMore(
							&lpDestRes->res.resComment.lpProp[i],
							lpSrcRes->res.resComment.lpProp,
							MAPIAllocateMore,
							lpParent));
					}
				}
