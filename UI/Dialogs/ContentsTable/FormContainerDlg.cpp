...
namespace dialog
{
	...
	void CFormContainerDlg::OnRemoveForm()
	{
		...
		MyClass.AddPane(0, viewpane::TextPane::CreateSingleLinePane(IDS_CLASS, false));
	...
	void CFormContainerDlg::OnResolveMultipleMessageClasses()
	{
		...
		MyData.AddPane(0, viewpane::TextPane::CreateSingleLinePane(IDS_NUMBER, false));
	...
	void CFormContainerDlg::OnGetDisplay()
	{
		...
		if (lpszDisplayName)
		{
			...
			MyOutput.AddPane(0, viewpane::TextPane::CreateSingleLinePane(IDS_GETDISPLAY, szDisplayName, true));
