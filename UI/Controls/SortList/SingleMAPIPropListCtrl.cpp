...
#pragma warning(pop)
		...
		void CSingleMAPIPropListCtrl::InitMenu(_In_ CMenu* pMenu) const
		{
			if (pMenu)
			{
				...
				pMenu->EnableMenuItem(
					ID_PASTE_PROPERTY,
					DIM(bHasSource && (ulStatus & BUFFER_PROPTAG) && (ulStatus & BUFFER_SOURCEPROPOBJ)));
		...
		_Check_return_ HRESULT CSingleMAPIPropListCtrl::LoadMAPIPropList()
		{
			...
			output::DebugPrintEx(
				output::DBGGeneric, CLASS, L"LoadMAPIPropList", L"added %u properties\n", ulCurListBoxRow);
		...
		void CSingleMAPIPropListCtrl::AddPropToExtraProps(ULONG ulPropTag, bool bRefresh)
		{
			...
			output::DebugPrintEx(
				output::DBGGeneric, CLASS, L"AddPropToExtraProps", L"adding proptag 0x%X\n", ulPropTag);
		...
		void CSingleMAPIPropListCtrl::OnOpenProperty() const
		{
			...
			if (SUCCEEDED(hRes) && lpProp)
			{
				...
				else if (PT_BINARY == PROP_TYPE(lpProp->ulPropTag) || PT_MV_BINARY == PROP_TYPE(lpProp->ulPropTag))
				{
					...
					case PT_MV_BINARY:
						output::DebugPrintEx(
							output::DBGGeneric, CLASS, L"OnOpenProperty", L"property is PT_MV_BINARY\n");
