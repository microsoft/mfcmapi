...
#define MAX_SORT_COLS 26
		...
		{
			output::DebugPrint(
				DBGMenu, L"CSingleMAPIPropListCtrl::HandleMenu wMenuSelect = 0x%X = %u\n", wMenuSelect, wMenuSelect);
			switch (wMenuSelect)
			{
		...
		void CSingleMAPIPropListCtrl::GetSelectedPropTag(_Out_ ULONG* lpPropTag) const
		{
			...
			{
				output::DebugPrintEx(
					DBGGeneric, CLASS, L"GetSelectedPropTag", L"returning lpPropTag = 0x%X\n", *lpPropTag);
			}
		}
						...
						if (m_lpPropBag->GetMAPIProp())
						{
							...
							if (ulNamedProps)
							{
								...
								if (lpTag)
								{
									...
									// Get the names
									WC_H_GETPROPS(cache::GetNamesFromIDs(
...
#define __UPPERBOUND 0xFFFF
		...
		void CSingleMAPIPropListCtrl::CountNamedProps()
		{
			...
			output::DebugPrintEx(
				DBGNamedProp, CLASS, L"CountNamedProps", L"Searching for the highest named prop mapping\n");
